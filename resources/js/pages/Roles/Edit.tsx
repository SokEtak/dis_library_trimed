...
export default function RolesEdit({
    ...
    const initialPermissions = Array.isArray(role.permissions)
        ? role.permissions.map((p) => p.id).filter((id) => id !== undefined)
        : [];
    ...
    useEffect(() => {
        ...
        console.log("Form data.permissions:", data.permissions);
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t.editBreadcrumb || "Edit Role", href: "" },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t.editTitle || "Edit Role"} />
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Input
                                                ...
                                                placeholder={t.editNamePlaceholder || "Enter pms name"}
                                        ...
                                        <TooltipContent className="bg-indigo-600 text-white rounded-lg p-2">
                                            {t.editNameTooltip || "Enter a unique name for the pms"}
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                                {errors.name && (
                                ...
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            ...
                                            >
                                                {processing ? t.editUpdating || "Updating..." : t.editUpdate || "Update"}
                                            </Button>
                                        </TooltipTrigger>
                                        <TooltipContent className="bg-indigo-600 text-white rounded-lg p-2">
                                            {t.editUpdateTooltip || "Save changes to the pms"}
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                                <TooltipProvider>
                                    <Tooltip>
                                        ...
                                        <TooltipContent className="bg-indigo-600 text-white rounded-lg p-2">
                                            {t.editCancelTooltip || "Return to roles list"}
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                            </div>
