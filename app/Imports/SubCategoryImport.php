...
class SubCategoryImport
{
    ...
    /**
     * Import subcategories from a CSV file path.
     *
        ...
        DB::transaction(function () use ($reader, &$result): void {
            foreach ($reader->getRecords() as $index => $row) {
                try {
                    ...
                    $subcategory = SubCategory::query()->where('name', $payload['name'])->first();
                    if ($subcategory) {
                        $subcategory->fill($payload)->save();
                        $result['updated']++;
                    } else {
